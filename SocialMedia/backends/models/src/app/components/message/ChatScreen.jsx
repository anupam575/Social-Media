...
useEffect(() => {
  ...
  fetchedOnceRef.current[conversationId] = true;
  setLoading(true);

  // 1️⃣ Fetch unread messages from server
  socket.emit("getUnreadMessages", { conversationIds: [conversationId] });

  // 2️⃣ Handle server response
  const handleUnreadMessages = async ({ messages: newMessages }) => {
    if (!Array.isArray(newMessages) || !newMessages.length) return;

    // 3️⃣ Filter messages sent by other user (receiver = current user)
    const filtered = newMessages.filter(
      (m) => String(getSenderId(m.senderId)) !== userId
    );
    ...
    // 4️⃣ Update local state (avoid duplicates)
    setMessages((prev) => {
    ...
    // 5️⃣ Mark messages as read on backend using handleMessageRead
    try {
      ...
      // 6️⃣ Optimistic UI update: mark as read immediately
      setMessages((prev) =>
        prev.map((m) =>
          filtered.some((fm) => fm._id === m._id) ? { ...m, read: true } : m
        )
      );
    } catch (err) {
      console.error("❌ Failed to mark unread messages as read via handleMessageRead:", err);
    }
  };
...
useEffect(() => {
  ...
  const handleMessageRead = ({ conversationId: convId, messageIds, readerId }) => {
    // Sirf current conversation ke liye handle kare
    ...
    // Optional: console.log(`Messages read by: ${readerId}`, messageIds);
  };
...
useEffect(() => {
  ...
  // 🔹 Fetch messages directly to local state
  useEffect(() => {
    if (!conversationId) return;
